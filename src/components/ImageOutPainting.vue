...
const createTask = async () => {
  ...
  if (res.data.code === 0 && res.data.data) {
    ...
    taskId.value = taskData?.output?.taskId || null
    // 开启轮询
    startPolling()
    await message.success('创建任务成功，请耐心等待，不要退出界面')
  } else {
    await message.error('创建任务失败，' + res.data.message)
  }
}
    ...
    try {
      ...
      if (res.data.code === 0 && res.data.data) {
        ...
        if (taskResult?.taskStatus === 'SUCCEEDED') {
          ...
          clearPolling()
          await message.success('扩图任务成功')
        } else if (taskResult?.taskStatus === 'FAILED') {
          clearPolling()
          await message.error('扩图任务失败')
        }
      }
    } catch (error) {
...
const handleUpload = async () => {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      // 关闭弹窗
      closeModal()
      await message.success('图片上传成功')
    } else {
      await message.error('图片上传失败，' + res.data.message)
    }
  } catch (error: any) {
    await message.error('图片上传失败' + (error?.message || '未知错误'))
  } finally {
