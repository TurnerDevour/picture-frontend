...
import { getSpaceCategoryAnalyzeUsingPost } from '@/api/spaceController'
...
const options = computed(() => {
  ...
  const sizeData = dataList.value.map((item) => (Number(item.totalSize) / (1024 * 1024)).toFixed(2)) // 转为 MB
...
const fetchData = async () => {
  ...
  if (res.data.code === 0 && res.data.data) {
    ...
    await message.error('获取资源分类使用分析数据失败，' + res.data.message)
  }
  loading.value = false
}
